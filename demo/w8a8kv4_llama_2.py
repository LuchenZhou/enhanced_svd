...
class LlamaForCausalLM(nn.Module):
    def __init__(
        ...
        svd_path: Optional[str] = None,  # 添加SVD路径
    ) -> None:
        super().__init__()
        self.config = config
        ...
        self.svd_path = svd_path  # Path for saving and loading SVD
        self.svd_right_vectors = None
        self.q_size = config.hidden_size  # Ensure q_size is initialized, assuming it's based on hidden_size
        self.kv_size = config.hidden_size  # Initialize kv_size, you can modify this based on actual needs

        if self.svd_path is not None:
            self.load_svd()

    def load_svd(self):
        """Load the saved right singular vectors (R) if available."""
        if os.path.exists(self.svd_path):
            self.svd_right_vectors = torch.load(self.svd_path)
            print(f"Loaded SVD right singular vectors from {self.svd_path}")
        else:
            print(f"Warning: SVD file {self.svd_path} not found!")

    def save_svd(self, qkv_matrix):
        """Perform SVD on the QKV matrix and save the right singular vectors (R)."""
        U, S, Vh = torch.svd(qkv_matrix)
        right_vectors = Vh.T  # The right singular vectors (R) are the columns of Vh
        torch.save(right_vectors, self.svd_path)
        print(f"Saved SVD right singular vectors to {self.svd_path}")

    def apply_svd_reduction(self, qkv_matrix):
        """Apply the precomputed SVD right singular vectors (R) to reduce the QKV dimensions."""
        if self.svd_right_vectors is not None:
            qkv_reduced = torch.matmul(qkv_matrix, self.svd_right_vectors)
            return qkv_reduced
        else:
            return qkv_matrix  # If no SVD applied, return the original matrix

    def extract_qkv_matrix(self, hidden_states):
        """Extract the QKV matrix from the hidden states."""
        # Assuming hidden states represent the QKV matrix
        # This can be customized to match the actual implementation
        qkv_matrix = hidden_states  # For simplicity, assume hidden states represent the QKV matrix
        return qkv_matrix
    ...
    def forward(
        ...
        # Extract the QKV matrix from the hidden states
        qkv_matrix = self.extract_qkv_matrix(hidden_states)

        # Apply SVD reduction (offline optimization)
        reduced_qkv = self.apply_svd_reduction(qkv_matrix)

        if input_metadata.is_prompt:
            output = self.lm_head(
                reduced_qkv[:, input_metadata.activation_buffer.batched_seq_len - 1:, :]
            )  # only compute last logits
        else:
            output = self.lm_head(reduced_qkv)
...
def reorder_linear_weights(
    ...
    # 动态计算 split_sizes
    total_size = weight.shape[1]  # 假设沿第1维进行切分
    num_parts = 3  # 假设你想分为3个部分

    # 动态计算每个部分的大小
    avg_split_size = total_size // num_parts
    split_sizes = [avg_split_size] * (num_parts - 1)  # 先分配平均大小
    split_sizes.append(total_size - sum(split_sizes))  # 剩余的部分放入最后一个

    # 确保 split_sizes 的和等于输入张量的大小
    assert sum(split_sizes) == total_size, f"Expected split_sizes to sum to {total_size}, but got {sum(split_sizes)}"
