...
def load_svd_compressed_weights(model, model_version):
    compressed_path = os.path.join("./svd_compressed_true_head_dim", model_version, "compressed_model.pth")
...
def find_decoder_layers(model):
    cand = [
        "model.layers",
        "model.layer",          # llama.cpp 等早期权重
        "layers",
        "model.decoder.layers",
        "model.decoder.layer",  # 少数 ORT/DeepSpeed 导出
    ]
    for path in cand:
        obj = model
        ok = True
        ...
        if ok and isinstance(obj, (list, torch.nn.ModuleList)):
            return obj          # 找到了
    # 全都没命中——打印线索再报错
    print("❌ cannot locate decoder layers.  Top-level attrs:", dir(model)[:50])
    raise RuntimeError("decoder layers not found")
def patch_head_dim32_all(model, new_head_dim=32):
    num_heads    = model.config.num_attention_heads        # 32
    num_kv_heads = model.config.num_key_value_heads        # 8
    dec_layers   = find_decoder_layers(model)              # ← 用探测函数

    for layer in dec_layers:
        sa = layer.self_attn

        # q_proj 4096→1024
        sa.q_proj = torch.nn.Linear(
            sa.q_proj.in_features,
            num_heads * new_head_dim,         # 32×32=1024
            bias=False, dtype=sa.q_proj.weight.dtype,
            device=sa.q_proj.weight.device)

        # k/v_proj 4096→256
        for name in ("k_proj", "v_proj"):
            old = getattr(sa, name)
            setattr(sa, name, torch.nn.Linear(
                old.in_features,
                num_kv_heads * new_head_dim,  # 8×32=256
                bias=False, dtype=old.weight.dtype,
                device=old.weight.device))

        # o_proj 1024→4096
        sa.o_proj = torch.nn.Linear(
            num_heads * new_head_dim,         # 1024
            sa.o_proj.out_features,           # 4096
            bias=False, dtype=sa.o_proj.weight.dtype,
            device=sa.o_proj.weight.device)
...
if __name__ == "__main__":
    ...
    kv_cache_head_dim = 32      # 记得同步 KV-cache 维度
    ...
    sa = model.model.layers[28].self_attn
    print(sa.k_proj.weight.shape)   # torch.Size([512, 4096])
    print(sa.head_dim)              # 128
    print(kv_cache_head_dim) 
    def func1():
        with torch.no_grad():
