...
import torch
from torch.linalg import svd  # 使用 torch.linalg.svd 进行奇异值分解
...
class LLMNeedleHaystackTester:
    ...
    def __init__(
        ...
        self.model_to_test = AutoModelForCausalLM.from_pretrained(
            model_name,
            torch_dtype=torch.float32,  # 使用 float32 以便进行 SVD
            attn_implementation="eager",
            output_hidden_states=False,  # 量化后无需隐藏状态
            return_dict=True,  # 确保返回字典
        ).eval()

        # 获取模型的最大序列长度
        self.max_length = self.model_to_test.config.max_position_embeddings
        print(f"Model max sequence length: {self.max_length}")

        # 应用 SVD 量化
        self.apply_svd_quantization(k=10)  # 选择保留前 10 个奇异值
    ...
    def evaluate_and_log(self, context_length, depth_percent):
        ...
        # 确保 context_length 不超过模型的最大序列长度减去提问长度
        context_length = min(context_length, self.max_length - self.simulation_length)
        ...
        # Convert the model to fp16 if necessary
        self.model_to_test = self.model_to_test.half()
        with torch.no_grad():
            if self.args.prefilling_chunk_size is not None:
                ...
                for i in range(
                    ...
                    chunk = chunk.long()  # Convert to Long type
                     # Ensure the chunk is in fp16
                    chunk = chunk.half()
                ...
                prompt_input_ids = prompt_input_ids.half()

                output = self.model_to_test(
                    input_ids=prompt_input_ids, past_key_values=None, use_cache=True, return_dict=True
                )
                past_key_values = output.past_key_values

            for input_id in question_input_ids[0]:
                input_id = input_id.unsqueeze(0).unsqueeze(0).half()
    ...
    def svd_quantize_weights(self, weight_matrix, k):
        """
        对权重矩阵进行 SVD 量化，保留前 k 个奇异值。

        :param weight_matrix: 原始权重矩阵 (torch.Tensor)
        :param k: 保留的奇异值数量
        :return: 量化后的权重矩阵 (torch.Tensor)
        """
        try:
            ...
            S_k = torch.diag(S[:k])
            Vh_k = Vh[:k, :]
            quantized_matrix = torch.mm(U_k, torch.mm(S_k, Vh_k))
            return quantized_matrix
        except Exception as e:
            print(f"SVD 量化失败: {e}")
            return weight_matrix  # 如果失败，返回原始权重

    def apply_svd_quantization(self, k=10):
        """
        对模型中的 Q、K、V 和 O 矩阵应用 SVD 量化。

        :param k: 保留的奇异值数量
        """
        for name, param in self.model_to_test.named_parameters():
            if any(proj in name for proj in ['q_proj', 'k_proj', 'v_proj', 'out_proj']):
                print(f"Applying SVD quantization to {name}")
                quantized_weight = self.svd_quantize_weights(param.data, k)
