...
python -u needle_in_haystack.py --s_len 0 --e_len 128000\
    ...
    --model_path /data2/xuezeyu/models/Llama-2-7B-32K-Instruct
...
python -u needle_in_haystack.py --s_len 0 --e_len 128000\
    --model_provider LLaMA\
    --model_path /data2/xuezeyu/models/Mistral-7B-Instruct-v0.2
...
python -u needle_in_haystack.py --s_len 0 --e_len 128000\
    --model_provider LLaMA\
    --model_path /data2/xuezeyu/models/Llama-3-8B-Instruct-Gradient-1048k-w8a8kv4-per-channel
...
def reset_rope(model, model_max_train_len, scaling_factor):
    for layer in model.model.layers:
        ...
        dtype = model.config.torch_dtype  # 继承模型主精度
        
        # 动态调整RoPE参数
        rotary_emb = layer.self_attn.rotary_emb
        rotary_emb.scaling_factor = scaling_factor
        
        # 兼容新版transformers的缓存生成方式
        if hasattr(rotary_emb, '_update_cos_sin_cache'):
            rotary_emb._update_cos_sin_cache(
                max_seq_len=model_max_train_len,
                device=device,
                dtype=dtype
            )
        else:  # 旧版兼容
            rotary_emb._set_cos_sin_cache(
                seq_len=model_max_train_len,
                device=device,
                dtype=dtype
            )
    return
...
class LLMNeedleHaystackTester:
    ...
    def __init__(self,
        ...
        if "Qwen" in self.model_version:
            ...
            self.model_to_test = MistralForCausalLM.from_pretrained(
                    model_name,torch_dtype="auto",device_map={"": 0},attn_implementation="flash_attention_2",trust_remote_code=True,
                ).eval()
            ...
            self.model_to_test = LlamaForCausalLM.from_pretrained(model_name,
                attn_implementation="flash_attention_2", torch_dtype=torch.bfloat16,device_map={"": 0}, max_position_embeddings=128000).eval()
    ...
    def decode(self, q_outputs, inp, decode_len, block_list=None):
        output = []
        retrieval_score = [[[0, ''] for _ in range(self.head_num)] for _ in range(self.layer_num)]
        ...
        # 确保 past_kv 在正确设备上
        past_kv = [
            tuple(x.to(device) for x in layer) for layer in past_kv
        ]
        ...
        for step_i in range(decode_len):
            inp = inp.view(1, 1).to(device)  # 确保每一步输入都在正确设备上
            ...
            past_kv = [
                tuple(x.to(device) for x in layer) for layer in outputs.past_key_values
            ]

            # 取 logits 最大值作为下一个 token
    ...
    def evaluate_and_log(self, context_length, depth_percent):
        ...
        max_length = self.model_to_test.config.max_position_embeddings
        if context_length > max_length:
            raise ValueError(f"Context length {context_length} exceeds model's max {max_length}")
        ...
        if self.model_version in ["Mistral-7B-Instruct-v0.2", "Qwen1.5-14B-Chat"]:
            ...
            input_ids = self.enc(input_context , return_tensors="pt",truncation=True, max_length=self.model_to_test.config.max_position_embeddings - 50 )['input_ids']
