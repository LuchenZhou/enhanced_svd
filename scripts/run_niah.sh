...
#CUDA_VISIBLE_DEVICES=0,1,2,3,4,5 bash scripts/niah.sh $attn_pattern $sparsity $model $context_lengths_min 0 $pretrained_len $model_provider
CUDA_VISIBLE_DEVICES=3 bash scripts/niah.sh $attn_pattern $sparsity $model $context_lengths_min 0 $pretrained_len $model_provider 
'''
